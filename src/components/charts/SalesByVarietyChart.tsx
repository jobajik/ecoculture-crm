...
import type { SalesByVarietyRow } from "@/lib/analytics";
...
export default function SalesByVarietyChart({ data }: { data: SalesByVarietyRow[] }) {
  const top = data.slice(0, 12);
  return (
    <ChartCard title="Продажи по сортам" description="Количество проданных стеблей (кроме отменённых заявок)">
      ...
      <ResponsiveContainer>
        <BarChart data={top} margin={{ top: 4, right: 8, left: 0, bottom: 24 }}>
          ...
          <Tooltip contentStyle={tooltipStyle} formatter={(value: number) => [`${value} шт.`, "Продано"]} />
          <Bar dataKey="totalQuantity" radius={[4, 4, 0, 0]} maxBarSize={36}>
            {top.map((row) => (
