 ...
 *
 * Присланное тем, кому не положено, отбрасывается молча. Это не поблажка, а
 * починка настоящей поломки: форма подставляет направление по городу клиента
 * («Бишкек» → «Киргизия»), но менеджеру поля не показывает. Пока здесь стоял
 * отказ, менеджер не мог оформить заявку НИ ОДНОМУ клиенту из региона — он
 * получал «Направление ставит руководитель отдела продаж» и никак не мог это
 * исправить, потому что поля не видел. Владелец упёрся в это на клиенте из
 * Бишкека.
 *
 * Заявка менеджера и должна приходить без направления: РОП помечает такие сам
 * одним нажатием в своём разделе (`ordersMissingDirection`).
 */
