 ...
 *
 * 1. **Направление в клиентской заявке ставит САМ МЕНЕДЖЕР** — «пусть менеджер
 *    сам заявку по Киргизии делает, без РОПа». Сначала было наоборот: поле
 *    видел только РОП, а заявки менеджеров он размечал вручную по списку
 *    «похоже на регион, но направление не стоит». Выходило, что человек,
 *    который знает, куда едет цветок, сообщить об этом не мог, а тот, кто не
 *    знает, должен был догадаться. Объём НА ГОРОД (заявка без клиента) — это
 *    по-прежнему работа РОПа, это другая история и другая форма.
...
/**
 * Кто ставит направление отгрузки.
 *
 * **Менеджер — сам, без РОПа.** Так решил владелец: «Пусть менеджер сам заявку
 * по Киргизии делает. Без РОПа». Сначала направление было только у РОПа, и
 * получалось глупо: менеджер договорился с клиентом из Бишкека, оформил заявку,
 * а в план отгрузок она попадала только после того, как РОП это заметит и
 * пометит вручную. То есть человек, который знает, куда едет цветок, сообщить
 * об этом не мог, а тот, кто не знает, должен был догадаться.
 *
 * РОП и администратор — тоже: РОП ведёт объём на город, администратор чинит
 * старые заявки.
 *
 * А вот у розницы и склада направления не бывает совсем, и это не забытая
 * роль: заявка в наш магазин — перемещение внутри компании, направление
 * отгрузки в плане регионального ОПТА к ней не относится.
 */
...
/**
 * Кто видит раздел «Регионы» — рабочее место РОПа, план против факта по всем
 * менеджерам сразу.
 *
 * Раньше это была та же проверка, что и «кто ставит направление». Развязано
 * намеренно: менеджер теперь направление ставит, но чужие цифры по городам ему
 * ни к чему — это разговор РОПа с владельцем.
 */
 ...
 */
/**
 * Какое направление РЕАЛЬНО попадёт в заявку от этого человека.
 *
 * Присланное тем, кому не положено, отбрасывается МОЛЧА, а не отвергается. Это
 * не поблажка, а починка настоящей поломки: форма подставляет направление по
 * городу клиента («Бишкек» → «Киргизия») и отправляет его всегда, даже если
 * поля человек не видел. Пока здесь стоял отказ, менеджер не мог оформить
 * заявку НИ ОДНОМУ клиенту из региона: он получал «Направление ставит
 * руководитель отдела продаж» и никак не мог это исправить — поля-то нет.
 * Владелец упёрся в это на клиенте из Бишкека.
 *
 * Сейчас менеджер направление видит и ставит сам, и функция осталась ровно для
 * тех, у кого направления не бывает вовсе, — розницы и склада: их формы про это
 * поле ничего не знают, но общий код заявки может его донести.
 */
export function directionFor(
  role: string | null | undefined,
  submitted: string | null | undefined
): string {
  return canSetDirection(role) ? cleanDirection(submitted) : "";
}
...
export function directionRefusal(input: {
  ...
  if (!canSetDirection(input.role)) {
    return "Эта роль оформляет заявки только на наши магазины, направления отгрузки у них нет";
  }
  if (input.isShop) {
...
/**
 * Почему направление УЖЕ ОФОРМЛЕННОЙ заявки поменять нельзя. Пусто — можно.
 *
 * Отдельно от `directionRefusal`, потому что вопрос другой: там «можно ли
 * прислать это значение», здесь «можно ли трогать ЭТУ заявку». Раз менеджер
 * направление ставит сам, он должен и уметь его исправить: иначе ошибка в
 * выпадающем списке снова упиралась бы в РОПа, а ради этого всё и менялось.
 *
 * Границы:
 *
 * - **менеджер правит только свою заявку.** Чужая — это чужие цифры в плане
 *   города и чужой разговор с клиентом. РОП и администратор правят любую: РОП
 *   сводит план по всем менеджерам, администратор чинит старое;
 * - **у городской заявки направление НЕ МЕНЯЕТСЯ.** Город там и есть
 *   контрагент: «объём на Астану» с направлением «Семей» — это другая заявка,
 *   а не правка этой. То же правило, по которому в правке не меняется клиент;
 * - **у заявки в наш магазин направления не бывает** — это перемещение внутри
 *   компании.
 *
 * Проверка на сервере, а не в форме: серверное действие вызывается обычным
 * запросом, и спрятанное поле ничего не запрещает (грабли 1.11).
 */
export function directionEditRefusal(input: {
  role: string | null | undefined;
  /** Кто нажимает. */
  actorEmail: string;
  /** Чья заявка. */
  orderManagerEmail: string;
  direction: string;
  isShop: boolean;
  /** Городская заявка (объём на регион)? */
  isRegion: boolean;
}): string {
  if (input.isRegion) {
    return (
      "У заявки на объём в город направление — это и есть город, менять его нельзя. " +
      "Нужен другой город — заведите заявку на него."
    );
  }
  if (input.isShop) {
    return "Заявка в наш магазин — это перемещение внутри компании, направления отгрузки у неё нет";
  ...
  if (!canSetDirection(input.role)) {
    return "Направление отгрузки ставит менеджер заявки или руководитель отдела продаж";
  }
  const all = input.role === ROLES.SALES_HEAD || input.role === ROLES.ADMIN;
  if (!all && input.actorEmail.trim().toLowerCase() !== input.orderManagerEmail.trim().toLowerCase()) {
    return "Это заявка другого менеджера — направление в ней ставит он или руководитель отдела продаж";
  }
  if (input.direction && !cleanDirection(input.direction)) return "Неизвестное направление";
  return "";
}
