...
export const STAGE_LABELS: Record<PaymentStage, string> = {
  // Не «счёт не отправлен», а «отметки нет» — и это решение владельца, а не
  // придирка к слову. Колонка отметки появилась в сентябре: у всех прежних
  // заявок она пуста, и назвать их неотправленными значило бы соврать про
  // счета, которые Юлия давно отправила руками (грабли 1.10 — пустая ячейка
  // ничего не утверждает). Бухгалтер проставляет отметку по мере работы, и
  // список становится честным сам собой.
  new: "Отметки о счёте нет",
...
export const STAGE_HINTS: Record<PaymentStage, string> = {
  ...
  new: "Отметки об отправке счёта нет. Отправили — нажмите «счёт отправлен»; не отправили — напишите почему",
...
/**
 * Что записать в заявку, когда бухгалтер ставит или снимает отметку о счёте.
 *
 * Чистая функция, потому что правил тут два и оба легко нарушить:
 *
 * 1. **повторная отметка не двигает дату** — она про ПЕРВУЮ отправку счёта, и
 *    это главный вопрос в разговоре с должником: когда мы вообще напоминали;
 * 2. **заметка «почему счёт не ушёл» стирается при отправке.** Она объясняла,
 *    почему счёт лежит; счёт ушёл — объяснять нечего, а оставленное «ждём
 *    реквизиты» рядом с отправленным счётом через месяц было бы враньём.
 *
 * Снятие отметки стирает дату целиком: полуправда «отправляли, но когда, не
 * знаем» хуже честного «отметки нет».
 */
export function invoiceFieldsOnSent(
  sent: boolean,
  current: { invoiceSentAt?: string }
): { invoiceSentAt: string; invoiceNote?: string } {
  if (!sent) return { invoiceSentAt: "" };
  return {
    invoiceSentAt: String(current.invoiceSentAt ?? "").trim() || new Date().toISOString(),
    invoiceNote: "",
  };
}
