...
export const SHEET_HEADERS: Record<string, string[]> = {
  ...
  [SHEET_TABS.ORDERS]: [
    ...
    "InvoiceSentAt",
    // Почему счёт ещё не ушёл — короткая заметка бухгалтера: «неверный
    // телефон», «ждём реквизиты». Владелец попросил её сразу после того, как
    // два счёта застряли из-за неправильных номеров: отметки об отправке не
    // было у многих строк, и какие именно застряли — приходилось вспоминать.
    //
    // Заметка живёт только до отправки: отметил счёт отправленным — она
    // стирается сама. Иначе через месяц рядом с отправленным счётом висело бы
    // «ждём реквизиты», и верить такой подписи было бы нельзя.
    //
  ...
  [SHEET_TABS.STAFF_TAKEOUTS]: [
    ...
    "WarehouseEmail",
    "Note",
    // Вид выдачи, ПОСЛЕДНЕЙ колонкой (грабли 1.1): пусто — сотруднику в счёт
    // зарплаты, «company» — на нужды компании (админ. расход).
    "Kind",
  ],
  ...
  [SHEET_TABS.LEADS]: [
    ...
    "LostReason",
    "ClientID",
    // Обзвон (сентябрь 2026): добавлены В КОНЕЦ (грабли 1.1). Campaign — название
...
/**
 * Вид выдачи со склада без продажи (вкладка StaffTakeouts, колонка Kind).
 *
 * «Нужды компании» (зав. складом называет это «админ расход») — подарки,
 * офис, мероприятия, образцы клиентам. Цветок ушёл бесплатно: это не продажа,
 * не потеря (списание) и не удержание из зарплаты. Лежит в той же вкладке, что
 * выдачи сотрудникам, потому что механика одна — партия, стебли, дата, — а
 * отличается только тем, кому и зачем.
 */
...
/** Подсказки для поля «На что» — ввод свободный, это только готовые слова. */
export const COMPANY_USE_PURPOSES = [
  "Подарок",
  "Офис",
  "Мероприятие",
  "Образцы клиенту",
