...
export interface Client {
  ...
  paymentMethod: string;
  /**
   * На какой наш Kaspi Pay переводит: «1» (Rose Farm), «2» (Есентай) или
   * «both» — по цветку в заявке. Счета выставляются от разных компаний.
   */
  kaspiAccount: string;
  /** С какого номера приходит платёж — по нему бухгалтер узнаёт перевод. */
  kaspiPhone1: string;
  /** Второй номер: у клиента бывает несколько каспи. */
  kaspiPhone2: string;
}
