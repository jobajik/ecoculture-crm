...
async function main() {
  ...
  // --- Просрочка считается по ЗАЯВКАМ, а не по клиенту целиком -------------
  //
  // Так уже было неправильно: у клиента брали возраст самой старой заявки и
  // записывали в просрочку ВЕСЬ его долг. На экране «Всего долг» и «Просрочено»
  // показывали одно и то же число, а в списке под ними половина строк говорила
  // «срок ещё не вышел». Отдельный набор заявок — чтобы не сдвинуть проверки
  // выше: они считают итоги по своему набору.
  const overdueOrders = [
    // Доставка шесть дней назад — просрочена (порог 3 дня).
    order("OLD", "Семей", "m1@x.kz", 100, 800, 0, { delivery: day(-6) }),
    // Вчерашняя доставка того же клиента — срок ещё не вышел.
    order("NEW", "Семей", "m1@x.kz", 100, 200, 0, { delivery: day(-1) }),
  ];
  const s2 = await getFinanceSnapshot("month", day(0), NOW, {
    orders: overdueOrders as never,
    users,
  });
  check("долг клиента — обе заявки", s2.debtTotal, 100_000);
  check("просрочена только старая заявка", s2.debtOverdueTotal, 80_000);
  check("у строки клиента видно просроченную часть", s2.debts[0].overdueAmount, 80_000);
  check("клиент помечен просроченным", s2.debts[0].overdue, true);
  check(
    "просроченное не может быть больше долга",
    s2.debts.every((d) => d.overdueAmount <= d.amount),
    true
  );

  // Свежий долг без единой просроченной заявки не должен красить карточку.
  const freshOnly = [order("FRESH", "Тараз", "m1@x.kz", 100, 300, 0, { delivery: day(-1) })];
  const s3 = await getFinanceSnapshot("month", day(0), NOW, {
    orders: freshOnly as never,
    users,
  });
  check("свежий долг в просрочку не идёт", s3.debtOverdueTotal, 0);
  check("свежий долг сам по себе есть", s3.debtTotal, 30_000);
